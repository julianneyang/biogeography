...
ucla_v_genera <- readr::read_rds(here("ImmDef-Mouse-Biogeography-Analysis/Taxa-Barplots/assign_cols.RDS"))
  ...
  print(ucla_v_genera)
ucla_o_genera <- readr::read_rds(here("Regional-Mouse-Biogeography-Analysis/2021-8-Microbiome-Batch-Correction-Analysis/Taxa-Barplots/assign_cols.RDS"))
  ucla_o_genera <- names(ucla_o_genera)
  print(ucla_o_genera)
  ucla_o_genera <- gsub("eae\\..","eae (", ucla_o_genera)
  ucla_o_genera <- gsub("f\\.","f)", ucla_o_genera)
spf_gavage <- readr::read_rds(here("Humanized-Biogeography-Analysis/Source RPCA/SPF/barplots/assign_cols.RDS"))
  spf_gavage <- names(spf_gavage)
  print(spf_gavage)
hum_gavage <- readr::read_rds(here("Humanized-Biogeography-Analysis/Source RPCA/Hum/barplots/assign_cols.RDS"))
