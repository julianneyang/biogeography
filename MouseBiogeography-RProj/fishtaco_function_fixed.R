...
MultiFunctionTaxaContributionPlots <- function(input_dir=NULL,
  ...
  if (!is.null(input_taxa_taxonomy)) {
    taxonomy = read_delim(here((paste0(fp,"../fish_taco/taxonomy.tsv"))))
    taxonomy <- taxonomy %>% column_to_rownames("Feature.ID")
    names(taxonomy) = c('kingdom','phylum','class','order','family','genus','species')
    taxonomy <- taxonomy %>%
      mutate(annotation = case_when(
        species == "s__" & genus == "g__" & family == "f__" ~ order,       # Condition 4
        species == "s__" & genus == "g__" ~ family,                        # Condition 3
        species == "s__" ~ genus,                                          # Condition 1
        species != "s__" ~ paste(genus, species)                           # Condition 2
      ))
  ...
  if (!is.null(input_taxa_taxonomy)) {
    ...
    if (length(strong_contributors) > 0) {
      for (i in 1:length(strong_contributors)) {
        ...
        curr_taxonomy_names = taxonomy[curr_taxa, "annotation"]
