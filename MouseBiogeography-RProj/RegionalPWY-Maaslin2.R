...
input_data <- read.csv("[Archive] clr Maaslin2 Pathway/PathwayCounts-All-Intestinal-Sites - colon-PWY.csv", header=TRUE, row.names=1) # choose filtered non rarefied csv file
input_data <- read.csv("[Archive] clr Maaslin2 Pathway/PathwayCounts-All-Intestinal-Sites - SI-PWY.csv", header=TRUE, row.names=1) # choose filtered non rarefied csv file

input_data <- read.csv("PathwayCounts-AllSubsets - Mucosal-PWY.csv", header=TRUE, row.names=1) # choose filtered non rarefied csv file
...
input_metadata <-read.csv("[Archive] clr Maaslin2 Pathway/Metadata-Regional_AllMetadata.csv",header=TRUE, row.names=1) #mapping file
...
warnings()
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, output = "CLR-Luminal-ComBat-Maaslin-SeqRunLineSexSite_General-1-MsID", fixed_effects = c("Sequencing_Run", "Line", "Sex", "Site_General"), random_effects = c("MouseID_Line"),normalization="clr", transform ="none")
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, output = "CLR-Colon-ComBat-Maaslin-SeqRunLineSexSiteType-1-MsID", fixed_effects = c("Sequencing_Run", "Line", "Sex", "Site", "Type"), random_effects = c("MouseID_Line"),normalization="clr", transform ="none")
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, output = "CLR-SI-ComBat-Maaslin-SeqRunLineSexSiteType-1-MsID", fixed_effects = c("Sequencing_Run", "Line", "Sex", "Site", "Type"), random_effects = c("MouseID_Line"),normalization="clr", transform ="none")
...
fit_data = Maaslin2(input_data=df_input_data, input_metadata=df_input_metadata, output = "LogCLR-Mucosal-ComBat-Maaslin-SeqRunSexSite-1-Line-MsID", fixed_effects = c("Sequencing_Run","Sex", "Site"), random_effects = c("Line", "MouseID_Line"),normalization="clr", transform ="LOG")
...
write.csv(Relative_Abundance,"Relative_Abundance-SI-Pathways.csv")
