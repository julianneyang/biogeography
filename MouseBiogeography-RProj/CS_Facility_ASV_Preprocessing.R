...
metadata<- read.csv(here("CS-Facility-Analysis/CS_Facility_Metadata - TL1a_Cedars_mapping_file.csv"))
...
metadata <- metadata %>%
  filter(Genotype=="WT") %>%
  filter(X.SampleID %in% names(ASV))
...
srun1 <- metadata%>% 
  filter(Sequencing_Run=="One", X.SampleID %in% names(ASV)) %>%pull(X.SampleID)
srun1 <- ASV[,srun1]


srun2 <- metadata%>% 
  filter(Sequencing_Run=="Two", X.SampleID %in% names(ASV)) %>%pull(X.SampleID)
srun2 <- ASV[,srun2]
